...
export function useDataStore(): DataStore {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.satellites?.himawari?.available === true) {
          try {
            const himawariCheck = await fetch(
              `${API_BASE_URL}/himawari/check-files`,
              {
                method: "POST",
            ...
            if (himawariCheck.ok) {
              const himawariData: FileCheckResponse =
                await himawariCheck.json();
              totalMissing +=
                himawariData.nc_files.missing.length +
        ...
        for (const satellite of sentinel3Satellites) {
          if (data.satellites?.[satellite]?.available === true) {
            try {
              ...
              const s3Check = await fetch(
                `${API_BASE_URL}/sentinel3/check-freshness`,
                {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({
            ...
            console.log(
              `${satellite} module not available, skipping freshness check`
            );
          }
        }
  ...
  const downloadLatestData = useCallback(async () => {
    try {
      ...
      console.log(
        `Available satellites (prioritized): ${sortedSatellites.join(", ")}`
      );
        ...
        try {
          ...
          if (satellite.startsWith("sentinel3")) {
            ...
            const response = await fetch(
              `${API_BASE_URL}/sentinel3/process-data`,
              {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
            ...
            if (response.ok) {
              ...
              if (result.task_id) {
                console.log(
                  `⏳ Waiting for ${satellite} processing to complete...`
                );
            ...
            console.log(
              `⏳ Starting Himawari data update (this may take longer due to network)...`
            );
            const response = await fetch(
              `${API_BASE_URL}/himawari/auto-monitor-repair`,
              {
                method: "POST",
                headers: { "Content-Type": "application/json" },
              }
  ...
  const pollSentinel3Status = useCallback(
    async (taskId: string, satellite: string) => {
      ...
      const poll = async () => {
        try {
          // 使用Sentinel-3的状态端点
          const response = await fetch(
            `${API_BASE_URL}/sentinel3/status/${taskId}`
          );
          if (response.ok) {
            const status = await response.json();
            console.log(
              `${satellite} processing status: ${status.status} - ${status.message}`
            );

            if (status.status === "completed") {
              console.log(
                `${satellite} data processing completed successfully!`
              );
              ...
              console.error(
                `${satellite} data processing failed:`,
                status.message
              );
              return;
  ...
  const updateData = useCallback(async () => {
    ...
    try {
        ...
        if (response.ok) {
          ...
          if (data.satellites?.himawari?.available === true) {
            ...
            if (himawariCheck.ok) {
              ...
              console.log(
                "Corrupted NC files:",
                himawariData.nc_files.corrupted
              );
            ...
            if (data.satellites?.[satellite]?.available === true) {
              try {
                ...
                if (s3Check.ok) {
                  ...
                  console.log(
                    `${satellite} freshness check: ${s3MissingCount} data types need updating`
                  );
                }
              } catch (e) {
                console.warn(
                  `Could not check ${satellite} freshness during update:`,
                  e
                );
              }
            }
      ...
      if (currentMissingFiles > 0) {
        console.log(
          `🔧 Found ${currentMissingFiles} missing files. Starting data download...`
        );
  ...
  const getParameterFiles = useCallback(
    async (paramId: string, fileType: "nc" | "png") => {
      try {
        const mapping =
          SATELLITE_MAPPING[paramId as keyof typeof SATELLITE_MAPPING];
        if (!mapping) {
        ...
        console.error(
          `Error fetching ${fileType} files for ${paramId}:`,
          error
        );
        return [];
      }
